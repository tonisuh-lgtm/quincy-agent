...
const seedTenants = () => {
  ...
  db.prepare(`INSERT OR IGNORE INTO tenants
    (id, name, short_name, phone, payment_method, rent, deposit, lease_start, lease_end, prorated_first, color, initials)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    'temi', 'Temitope W Onayemi', 'Temi', '+17089292466',
    'Apple Pay / Venmo', 1850, 1850, '2026-05-14', '2027-05-31', 1074.19, '#185FA5', 'TW'
  );

  db.prepare(`INSERT OR IGNORE INTO tenants
    (id, name, short_name, phone, payment_method, rent, deposit, lease_start, lease_end, prorated_first, color, initials)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    'chloe', 'Chloe Moslener', 'Chloe', '+18143924015',
    'Zelle', 1750, 1750, '2026-05-15', '2026-08-31', 960.48, '#0F6E56', 'CM'
  );
};
...
const seedConfig = () => {
  const defaults = [
    ...
    ['custom_rules', 'Keep SMS responses under 300 characters. Split longer messages naturally. Never be dismissive. If a tenant is upset or frustrated, acknowledge their feelings before asking questions. Sign off as: — 9 Quincy Management. Never discuss other tenants. Never reveal owner personal details.'],
  ];
